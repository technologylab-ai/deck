...
class Backend(Protocol):
    ...
    # close() is the inverse of open: quit an app target, or close the matched
    # browser tab. Driven by `deck close` (Stream Deck long-press).
    def close(self, handle: Handle) -> None: ...

    # Dry-run description: the exact command(s)/AppleScript that focus/create/
    # close would execute for this target, given a prior find() result.
